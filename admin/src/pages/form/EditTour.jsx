...
const EditTour = () => {
  ...
  const params = useParams(); //  dùng params lấy id Tour
  ...
  const urlFetch = `${requests.getTour}/${params.tourId}`;
  useEffect(() => {
    // fetch by id
    fetch(urlFetch)
      .then(res => {
      ...
      .then(data => {
        // console.log(data);
        setEnteredTourName(data.tour.name);
        setEnteredTime(data.tour.time);
        setEnteredCount(data.tour.count);

        setSelectedCategories(
          data.tour.category.map(cat => {
            return {
              value: cat._id,
              label: cat.name,
            };
        ...
        data.tour.discountId
          ? setSelectedDiscount({
              value: data.tour.discountId._id,
              label: `${data.tour.discountId.name} (${data.tour.discountId.percentOff}%)`,
            })
          : setSelectedDiscount({ value: null, label: 'None' });

        setEnteredAdultPrice(data.tour.adultPrice);
        setEnteredChildPrice(data.tour.childPrice);
        setEnteredBabyPrice(data.tour.babyPrice);

        setEnteredImg1(data.tour.img1);
        setEnteredImg2(data.tour.img2);
        setEnteredImg3(data.tour.img3);
        setEnteredImg4(data.tour.img4);

        setEnteredDesc(data.tour.desc);
        setEnteredWarn(data.tour.warn);
      })
  ...
  const submitHandler = async e => {
    ...
    // post update tour
    try {
      const res = await fetch(requests.postEditTour, {
        method: 'PUT',
        headers: { 'Content-type': 'application/json' },
        body: JSON.stringify({ tourId: params.tourId, ...newTour }),
        credentials: 'include',
      });

      if (res.ok) {
        dispatch(toastActions.SHOW_SUCCESS('Edit Tour successfully!')); // toast
  ...
  const resetFormHandler = () => {
    ...
    setEnteredAdultPrice('');
    setEnteredChildPrice('');
    setEnteredBabyPrice('');
    setEnteredImg1('');
    setEnteredImg2('');
    setEnteredImg3('');
  ...
  return (
    <Layout className="my-basic-form">
      <div className="top fw-bold">
        Update Tour
        <small className="text-muted"> {params.tourId}</small>
      </div>
      ...
      {!idValid && <div className="bottom">Tour này không tồn tại</div>}
    </Layout>
  );
};
