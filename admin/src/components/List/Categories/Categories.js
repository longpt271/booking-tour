...
const Categories = props => {
  ...
  const urlSearchFetch = requests.getCategories;
  const urlDelete = requests.deleteCategory;
  const urlDeleteMulti = requests.deleteCategories;
  ...
  useEffect(() => {
    setSelectedCategories([]); // set lại rỗng khi chuyển trang
  }, [pageNumber]);
  ...
  const handleSelectAllChange = () => {
    if (selectAll) {
      ...
      const allDiscountIds = dataFetch.map(cat => cat._id);
  ...
  useEffect(() => {
    if (selectedCategories.length === dataFetch.length) {
      setSelectAll(true);
    } else {
      setSelectAll(false);
    }
  }, [selectedCategories.length, dataFetch.length]);

  return (
    <div className="dataTable text-gray">
      <ListHeader name1="Categories" name2="Category" />
              ...
              return (
                <tr key={category._id}>
                  <td
                    ...
                    className={`cellWithStatus ${Boolean(category.isInTour)}`} // css kiểm tra đã trong tour nào chưa
                  >
                    <div className="d-flex">
                      <input
                        type="checkbox"
                  ...
                  <td className="small">{category._id}</td>
